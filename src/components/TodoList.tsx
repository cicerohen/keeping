...
interface TodoListProps {
  todos: Todo[]
  availableTags: Tag[]
  onTodoUpdate: (id: string, updates: Partial<Todo> & { tags?: Tag[] }) => Promise<void>
  onTodoDelete: (id: string) => Promise<void>
  onTagClick: (tag: string) => void
  viewMode: 'grid' | 'list'
}

export function TodoList({ todos, availableTags, onTodoUpdate, onTodoDelete, onTagClick, viewMode }: TodoListProps) {
  const [editingTodo, setEditingTodo] = useState<Todo | null>(null)

  if (todos.length === 0) {
    return <div className="text-center text-gray-500 py-8">No tasks matched your criteria.</div>
  }
        ...
        >
          {todo.image_url && (
            <div className="w-full h-32 mb-3 rounded-md overflow-hidden bg-gray-100 flex-shrink-0">
                <img src={todo.image_url} alt="Task attachment" className="w-full h-full object-cover" />
            </div>
          )}
